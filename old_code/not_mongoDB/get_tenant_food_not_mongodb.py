...
def save_to_mysql(items):
    ...
    start = time.time()
    # item_df = pd.DataFrame(columns=['food_id', 'business_id', 'price', 'name', 'image_path', 'month_saled',
    #                                 'description', 'specs_value', 'specs_features', 'md5', 'mt_poi_id'])
    count = 0
    ...
    # print(item_df)
    # for item in items:
    #     item_df = item_df.append(pd.Series(item, index=item_df.columns), ignore_index=True)
    #     count += 1
    #     if count % 10000 == 0:
    #         item_df = pd.DataFrame(items)
    #         print(item_df)
    #         print('已插入%s/%s条数据, 本次插入耗时%sS' % (count, items_count, time.time()-start))
...
def update_minus(minus_dic):
    ...
    # items = items.drop_duplicates(['business_id'])
...
async def spider():
    ...
    if url_item:
        ...
        if jsn['code'] == 801:
            ...
            # time.sleep(8)
            downloader.retry(url_item)
        file_name = str(url_item['post_data']['wm_poi_id']) + ".json"
